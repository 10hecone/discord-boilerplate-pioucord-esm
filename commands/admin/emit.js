...
export default {
    ...
    runInteraction(client, interaction) {
        const iEvent = interaction.options.getString('event');
        if(!eventList.some(e => e === iEvent)) return interaction.reply('```Error encountered.. sender is bad```');
        client.emit(iEvent).then(() => {
            return interaction.reply(`\`\`\`${iEvent} has been issued\`\`\``);
        }); 
    },
};
